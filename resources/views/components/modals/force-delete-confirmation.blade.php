...
<div x-show="showForceDeleteModal" x-transition:enter="transition ease-out duration-300"
    ...
    aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;" id="{{ $id }}">
    <div class="flex items-center justify-center min-h-screen p-4 text-center">
        <!-- Background overlay -->
        <div class="fixed bg-zinc-900/60 dark:bg-zinc-900/80 transition-opacity" @click="closeForceDeleteModal"
            aria-hidden="true"></div>

        <!-- Modal panel -->
        <div x-show="showForceDeleteModal" x-transition:enter="ease-out duration-300"
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="space-y-4">
                        <h3 class="text-2xl font-bold text-zinc-900 dark:text-white">
                            Deletion Blocked: Story Has Dependencies
                        </h3>
                        ...
                        <div class="space-y-3">
                            <p class="text-zinc-600/90 dark:text-zinc-300/90 leading-relaxed">
                                ...
                                @if ($itemName)
                                    <span
                                        class="block mt-2 text-lg font-bold bg-gradient-to-r from-amber-600 to-amber-500 bg-clip-text text-transparent">
                            ...
                            <div
                                class="mt-3 max-h-60 overflow-y-auto border border-zinc-200 dark:border-zinc-700 rounded-md">
                                    ...
                                    <template x-for="item in {{ $dependencies }}" :key="item.id">
                                        <li class="px-4 py-3 flex items-center text-sm">
                                            <i data-lucide="file-check-2"
                                                class="h-4 w-4 text-zinc-400 dark:text-zinc-500 mr-2"></i>
                            ...
                            <p class="mt-3 text-sm text-red-600 dark:text-red-400 font-medium">
                                These {{ $dependencyType }} must be reassigned to another story or deleted first.
                                You cannot delete a story with test cases attached...
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6 sm:px-8 sm:pb-8 border-t border-zinc-100/50 dark:border-zinc-700/50">
                <div class="flex gap-3 justify-end">
                    <button @click="closeForceDeleteModal()" type="button"
