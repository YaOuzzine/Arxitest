        ...
        <div class="auth-card bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-sm overflow-hidden">
            <div class="p-8">
                <div class="text-center mb-8">
                    <h1 class="text-2xl font-bold mb-2">Verify Your Phone</h1>
                    <p class="text-zinc-600 dark:text-zinc-400">Enter the verification code sent to your phone</p>
                </div>
                ...
                <form method="POST" action="{{ route('auth.phone.verify') }}" class="auth-form space-y-6" x-data="{ processing: false }">
                    @csrf

                    <input type="hidden" name="phone_number" value="{{ $phoneNumber ?? session('phone_number') }}">
                    ...
                    <div>
                        <button type="submit"
                            ...
                            <span x-show="!processing">Verify</span>
            ...
            <div class="px-8 py-4 bg-zinc-50 dark:bg-zinc-700/20 border-t border-zinc-200 dark:border-zinc-700 text-sm text-center">
                <span class="text-zinc-600 dark:text-zinc-400">Wrong phone number?</span>
                <a href="{{ route('auth.phone') }}" class="font-medium text-zinc-800 dark:text-zinc-200 hover:underline transition duration-150 ml-1">
                    Try again
                </a>
            </div>
        </div>
