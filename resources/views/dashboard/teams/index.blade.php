                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($teams as $team)
                        <x-entity-card
                            ...
                            :isHighlighted="$team->id === $currentTeamId"
                            :isCurrentTeam="$team->id === $currentTeamId"
                            :stats="[
                                ['icon' => 'users', 'value' => $team->users_count, 'label' => Str::plural('member', $team->users_count)],
                                ['icon' => 'folder', 'value' => $team->projects_count, 'label' => Str::plural('project', $team->projects_count)]
                            ]"
                            :badge="$team->id === $currentTeamId"
                            badgeLabel="Current"
                            :viewRoute="route('teams.show', $team->id)"
                            :editRoute="route('teams.edit', $team->id)"
                            :deleteAction="'openDeleteModal(\''. $team->id .'\', \''. addslashes($team->name) .'\')'"
                            :switchAction="'window.location.href=\''. route('dashboard.select-team') .'?team_id='. $team->id .'\''"
                            :footer="'Updated ' . $team->updated_at->diffForHumans()">
                        </x-entity-card>
                    @endforeach
                </div>
            @else
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-8 text-center">
                    <div class="mx-auto h-12 w-12 rounded-full bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center mb-4">
        ...
        Alpine.data('teamsManager', () => ({
            ...
            filterTeams() {
                ...
                teamCards.forEach(card => {
                    ...
                    if (teamName.includes(this.searchTerm) || teamDesc.includes(this.searchTerm)) {
            ...
            openDeleteModal(id, name) {
                console.log("Opening delete modal for team:", id, name); // Debug log
