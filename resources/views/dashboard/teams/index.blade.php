    ...
    <div class="h-full" x-data="teamsManager()">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex items-center">
                    <input type="text" x-model="searchQuery" placeholder="Search teams..."
                ...
                <a href="{{ route('teams.create') }}"
                    class="btn-primary flex items-center px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all duration-200 transform hover:scale-[1.02] shadow-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <template x-for="(team, index) in filteredTeams" :key="team.id">
                <div class="team-card bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-lg hover:translate-y-[-4px]"
                    :class="{ 'opacity-0 translate-y-8': !isTeamVisible(index) }" x-init="setTimeout(() => makeTeamVisible(index), 100 + (index * 100))">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center">
                                <template x-if="team.logo_path">
                                    <img :src="'/storage/' + team.logo_path" class="w-12 h-12 rounded-lg object-cover mr-4"
                                        :alt="team.name">
                                </template>
                                <template x-if="!team.logo_path">
                                    <div
                                        class="w-12 h-12 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-xl font-bold mr-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-zinc-500 dark:text-zinc-400"
                                        x-text="team.users_count + ' members'"></p>
                                </div>
                            </div>
                            <div x-data="{ menuOpen: false }" class="dropdown-container">
                                ...
                                <div x-show="menuOpen" @click.away="menuOpen = false"
                                    ...
                                    <div class="py-1">
                                        <a :href="'{{ url('/dashboard/teams') }}/' + team.id"
                                            class="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-600">
                                            <i data-lucide="eye" class="inline w-4 h-4 mr-2"></i> View Details
                                        </a>
                                        <template x-if="canEditTeam(team)">
                                            <a :href="'{{ url('/dashboard/teams') }}/' + team.id + '/edit'"
                                                class="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-600">
                        ...
                        <p class="text-zinc-600 dark:text-zinc-300 text-sm line-clamp-2 min-h-[40px] mb-4"
                            x-text="team.description || 'No description provided'"></p>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex -space-x-2">
                                <template x-for="(user, userIndex) in team.users.slice(0, 3)" :key="userIndex">
                                    <div class="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-700 border-2 border-white dark:border-zinc-800 flex items-center justify-center overflow-hidden"
                                        :title="user.name">
                                ...
                                <template x-if="team.users.length > 3">
                                    <div
                                        class="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-700 border-2 border-white dark:border-zinc-800 flex items-center justify-center text-xs font-medium text-zinc-600 dark:text-zinc-300">
                    ...
                    <div
                        class="px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-800/50 flex justify-between items-center">
                        ...
                        <form x-data="{ submitting: false }" @submit.prevent="selectTeam(team.id)">
                            <template x-if="isCurrentTeam(team)">
                                <span
                                    class="inline-flex items-center px-3 py-1 text-xs font-medium rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400">
                                    <i data-lucide="check" class="w-3 h-3 mr-1"></i> Current Team
                                </span>
                            </template>
                            <template x-if="!isCurrentTeam(team)">
                                <button type="submit"
                                    class="inline-flex items-center px-3 py-1 text-xs font-medium rounded-full bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-800/40 text-blue-800 dark:text-blue-400 transition-colors duration-200"
                                    :disabled="submitting">
        ...
        <div x-show="teams.length === 0"
            class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-8 text-center animate-fade-in">
        ...
        <div x-show="isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <template x-for="i in 3" :key="i">
                <div
                    class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden animate-pulse">
                    <div class="p-6">
        ...
        <x-modals.delete-confirmation title="Delete Team" message="Are you sure you want to delete the team"
            itemName="teamToDelete?.name"
            dangerText="This action cannot be undone and all team data will be permanently deleted."
            confirmText="Delete" />
