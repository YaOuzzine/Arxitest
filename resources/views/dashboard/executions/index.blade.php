...
<div class="space-y-6" x-data="executionsPage()">
    ...
    <x-index-header
        ...
        description="Monitor and manage your automated test runs"
        :createRoute="route('dashboard.executions.create')"
        createText="Run New Test"
        createIcon="play-circle">

        <!-- Optional filters section -->
        <x-slot:filters>
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div>
                    <label for="status-filter" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Status</label>
                    <select id="status-filter" name="status" class="w-full rounded-lg border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    <label for="script-filter" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Test Script</label>
                    <select id="script-filter" name="script_id" class="w-full rounded-lg border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="">All Scripts</option>
                        @foreach($scripts ?? [] as $script)
                            <option value="{{ $script->id }}" {{ request('script_id') === $script->id ? 'selected' : '' }}>
                                {{ $script->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="environment-filter" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Environment</label>
                    <select id="environment-filter" name="environment_id" class="w-full rounded-lg border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="">All Environments</option>
                        @foreach($environments ?? [] as $environment)
                            <option value="{{ $environment->id }}" {{ request('environment_id') === $environment->id ? 'selected' : '' }}>
                                {{ $environment->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="date-filter" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Time Period</label>
                    <select id="date-filter" name="time_period" class="w-full rounded-lg border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="flex justify-end mt-4">
                <button type="button" @click="applyFilters()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-2">
                    <i data-lucide="filter" class="w-4 h-4"></i>
                    Apply Filters
                </button>
                <button type="button" @click="resetFilters()" class="ml-2 px-4 py-2 border border-zinc-300 dark:border-zinc-600 text-zinc-700 dark:text-zinc-300 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-700 flex items-center gap-2">
                    ...
                    @forelse($executions as $execution)
                        <tr id="execution-row-{{ $execution->id }}"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600 dark:text-indigo-400">
                                <a href="{{ route('dashboard.executions.show', $execution->id) }}" class="hover:underline">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap" x-ref="statusCell">
                                <span x-show="statusName === 'pending'" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">
                                    Pending
                                </span>
                                <span x-show="statusName === 'running'" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                ...
                                <span x-show="statusName === 'completed'" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">
                                    Completed
                                </span>
                                <span x-show="statusName === 'failed'" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">
                                    Failed
                                </span>
                                <span x-show="statusName === 'aborted'" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400">
                                    Aborted
                                </span>
                                <span x-show="!['pending', 'running', 'completed', 'failed', 'aborted'].includes(statusName)" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-zinc-100 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-300">
                                    {{ $execution->status->name ?? 'Unknown' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-400" x-ref="startTimeCell">
                                {{ $execution->start_time ? $execution->start_time->diffForHumans() : 'Not started' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-400" x-ref="durationCell">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right space-x-2">
                                <a href="{{ route('dashboard.executions.show', $execution->id) }}" class="px-2 py-1 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 rounded hover:bg-indigo-200 dark:hover:bg-indigo-800/50 inline-flex items-center">
                                    <i data-lucide="eye" class="w-4 h-4 mr-1"></i> View
                                </a>
                                <button x-show="isRunning" @click="abortExecution('{{ $execution->id }}')" class="px-2 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded hover:bg-red-200 dark:hover:bg-red-800/50 inline-flex items-center">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-medium text-zinc-900 dark:text-zinc-200 mb-1">No test executions found</h3>
                                    <p class="text-zinc-500 dark:text-zinc-400 max-w-sm">
                                        Run your first test to see results here.
                                    </p>
                                    <a href="{{ route('dashboard.executions.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
...
<div x-data="{ show: false, executionId: null }" x-show="show"
     @open-abort-modal.window="show = true; executionId = $event.detail.id"
    ...
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-zinc-800 rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <div class="px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 sm:px-6 sm:flex sm:flex-row-reverse border-t border-zinc-200 dark:border-zinc-700">
                <button type="button" @click="confirmAbort()" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                    Abort
                </button>
    ...
    function executionsPage() {
        return {
            ...
            applyFilters() {
                ...
                const scriptId = document.getElementById('script-filter').value;
                const environmentId = document.getElementById('environment-filter').value;
                const timePeriod = document.getElementById('date-filter').value;
                ...
                if (scriptId) url.searchParams.set('script_id', scriptId);
                ...
                if (timePeriod) url.searchParams.set('time_period', timePeriod);
                else url.searchParams.delete('time_period');
            ...
            startStatusPolling(executionId) {
                ...
                this.timerIntervals[executionId] = setInterval(() => {
                    ...
                    liveTimer.textContent = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
                }, 1000);
