...
@section('content')
    <div class="space-y-6" x-data="executionDetails({
        ...
        aiReport: {{ isset($aiReport) ? json_encode($aiReport) : 'null' }}
    })">
        <!-- Execution Header -->
        <div
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex justify-between items-center">
                ...
                <div>
                    ...
                    <span x-show="statusName === 'pending'"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">
                        Pending
                    </span>
                    <span x-show="statusName === 'running'"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                    ...
                    <span x-show="statusName === 'completed'"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">
                        Completed
                    </span>
                    <span x-show="statusName === 'failed'"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">
                        Failed
                    </span>
                    <span x-show="statusName === 'aborted'"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400">
                        Aborted
                    </span>
                    <span x-show="!['pending', 'running', 'completed', 'failed', 'aborted'].includes(statusName)"
                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-zinc-100 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-300">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-4">
                    ...
                    <dl class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-zinc-900 dark:text-zinc-200">
                                {{ $execution->initiator->name ?? 'System' }}</dd>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-zinc-900 dark:text-zinc-200">
                                {{ $execution->start_time ? $execution->start_time->format('M j, Y g:i:s A') : 'Not started' }}
                            </dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-sm font-medium text-zinc-500 dark:text-zinc-400">End Time:</dt>
                            <dd class="text-sm text-zinc-900 dark:text-zinc-200" x-ref="endTimeDisplay">
                                {{ $execution->end_time ? $execution->end_time->format('M j, Y g:i:s A') : 'Running...' }}
                            </dd>
                        </div>
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-zinc-900 dark:text-zinc-200" x-ref="durationDisplay">
                                <span
                                    x-show="!isRunning">{{ $execution->duration ? gmdate('H:i:s', $execution->duration) : '-' }}</span>
                ...
                <div class="space-y-4 md:col-span-2">
                    ...
                    <div
                        class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-600/50">
                        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @forelse($execution->containers as $container)
                                    <tr>
                                        <td class="px-4 py-3 text-sm font-medium text-zinc-900 dark:text-zinc-200">
                                            {{ substr($container->container_id, 0, 12) }}</td>
                                        <td class="px-4 py-3">
            ...
            <div class="px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-3 justify-end">
                @if ($execution->isRunning())
                    <button @click="abortExecution()"
                        class="px-3 py-2 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-md hover:bg-red-200 dark:hover:bg-red-800/50 inline-flex items-center">
                        <i data-lucide="square" class="w-4 h-4 mr-2"></i> Abort Execution
                    </button>
                @endif
                <a href="{{ route('dashboard.executions.logs.download', $execution->id) }}"
                    class="px-3 py-2 bg-zinc-100 text-zinc-700 dark:bg-zinc-700/30 dark:text-zinc-300 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-700/50 inline-flex items-center"
                    x-bind:class="{ 'opacity-50 pointer-events-none': !logFileExists }">
                    <i data-lucide="download" class="w-4 h-4 mr-2"></i> Download Logs
                </a>
                <a href="{{ route('dashboard.executions.index') }}"
                    class="px-3 py-2 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 rounded-md hover:bg-indigo-200 dark:hover:bg-indigo-800/50 inline-flex items-center">
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <button @click="refreshLogs"
                        class="px-2 py-1 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 rounded hover:bg-indigo-200 dark:hover:bg-indigo-800/50 inline-flex items-center text-xs">
            ...
            <div class="p-6">
                ...
                <div class="bg-zinc-900 rounded-lg overflow-hidden text-white font-mono text-sm">
                    ...
                    <div class="relative">
                        <pre id="log-container" x-ref="logContainer" class="p-4 overflow-auto max-h-96 text-zinc-300 whitespace-pre-wrap"
                            style="font-size: 0.85rem; line-height: 1.5;" x-html="formattedLogs"></pre>

                        <!-- Loading Overlay -->
                        <div x-show="isLoadingLogs"
                            class="absolute inset-0 bg-zinc-900/80 flex items-center justify-center">
                            <div class="flex flex-col items-center">
                                <svg class="animate-spin h-8 w-8 text-indigo-400" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <!-- Empty State -->
                        <div x-show="!isLoadingLogs && (!logs || logs.trim() === '')"
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-zinc-600 text-xs mt-1" x-show="isRunning">Logs will appear as execution
                                    progresses.</p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white dark:bg-zinc-800 shadow-md rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700"
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- CPU Usage Chart -->
                    <div
                        class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg p-4 border border-zinc-200 dark:border-zinc-600/50">
        ...
        <div class="bg-white dark:bg-zinc-800 shadow-md rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700"
            x-show="aiReport !== null" x-cloak>
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex justify-between items-center">
                <h2 class="text-lg font-medium text-zinc-800 dark:text-zinc-200">
                    <i data-lucide="brain-circuit" class="w-5 h-5 text-indigo-600 dark:text-indigo-400 inline mr-2"></i>
                    AI Test Analysis
                </h2>
                <div>
                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full"
                        :class="{
                            'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400': aiReport
                                .status === 'passed',
                            'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400': aiReport
                                .status === 'failed',
                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400': aiReport
                                .status === 'inconclusive',
                            'bg-zinc-100 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-300': aiReport
                                .status === 'error'
                        }">
                        <span
                            x-text="aiReport.status === 'passed' ? 'Passed' :
                             aiReport.status === 'failed' ? 'Failed' :
                             aiReport.status === 'inconclusive' ? 'Inconclusive' : 'Error'"></span>
                        <span class="ml-1" x-show="aiReport.confidence">
                            (<span x-text="Math.round(aiReport.confidence * 100) + '%'"></span> confidence)
                        </span>
                    </span>
                </div>
            </div>
            ...
            <div class="p-6 space-y-6">
                <!-- Summary section -->
                <div class="space-y-3" x-show="aiReport.summary">
                    <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400"
                        x-text="aiReport.summary.title"></h3>
                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 p-4 rounded-md border border-indigo-100 dark:border-indigo-800/50"
                        x-html="aiReport.summary.content"></div>
                </div>

                <!-- Comparison section -->
                <div class="space-y-3" x-show="aiReport.comparison">
                    <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400"
                        x-text="aiReport.comparison.title"></h3>
                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 p-4 rounded-md border border-indigo-100 dark:border-indigo-800/50"
                        x-html="aiReport.comparison.content"></div>
                </div>

                <!-- Data analysis section -->
                <div class="space-y-3" x-show="aiReport.data_analysis">
                    <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400"
                        x-text="aiReport.data_analysis.title"></h3>
                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 p-4 rounded-md border border-indigo-100 dark:border-indigo-800/50"
                        x-html="aiReport.data_analysis.content"></div>
                </div>

                <!-- Issues section -->
                <div class="space-y-3" x-show="aiReport.issues">
                    <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400" x-text="aiReport.issues.title">
                    </h3>
                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 p-4 rounded-md border border-indigo-100 dark:border-indigo-800/50"
                        x-html="aiReport.issues.content"></div>
                </div>

                <!-- Evaluation section -->
                <div class="space-y-3" x-show="aiReport.evaluation">
                    <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400"
                        x-text="aiReport.evaluation.title"></h3>
                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 p-4 rounded-md border border-indigo-100 dark:border-indigo-800/50"
                        x-html="aiReport.evaluation.content"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div x-data="{ show: false }" x-show="show" @open-abort-modal.window="show = true"
        ...
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="px-4 pt-5 pb-4 bg-white dark:bg-zinc-800 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                    Are you sure you want to abort this test execution? This action cannot be undone and may
                                    result in incomplete test results.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div
                    ...
                    <button type="button" @click="$dispatch('confirm-abort'); show = false"
            ...
            return {
                ...
                async refreshLogs() {
                    try {
                        ...
                        const response = await fetch(
                            `/dashboard/executions/${this.executionId}/logs?offset=0&limit=1000`);
                ...
                async loadMoreLogs() {
                    try {
                        ...
                        const response = await fetch(
                            `/dashboard/executions/${this.executionId}/logs?offset=${this.logOffset}&limit=1000`);
