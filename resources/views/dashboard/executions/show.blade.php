...
<div x-data="{ activeTab: 'overview' }">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                {{ $execution->testScript->name ?? 'Unknown script' }} on {{ $execution->environment->name ?? 'Unknown environment' }}
            </p>
        </div>

        <div class="flex items-center space-x-3">
            @if($execution->isRunning())
                <form action="{{ route('dashboard.executions.abort', $execution->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to abort this execution?')">
                    @csrf
    ...
    <div class="mb-6 border-b border-zinc-200 dark:border-zinc-700">
        <ul class="flex flex-wrap -mb-px">
            <li class="mr-2">
                <button @click="activeTab = 'overview'" :class="{ 'border-indigo-500 text-indigo-600 dark:text-indigo-400': activeTab === 'overview', 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300': activeTab !== 'overview' }" class="inline-block p-4 border-b-2 font-medium">
                    Overview
                </button>
            </li>
            <li class="mr-2">
                <button @click="activeTab = 'containers'" :class="{ 'border-indigo-500 text-indigo-600 dark:text-indigo-400': activeTab === 'containers', 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300': activeTab !== 'containers' }" class="inline-block p-4 border-b-2 font-medium">
                    Containers
                </button>
            </li>
            <li class="mr-2">
                <button @click="activeTab = 'logs'" :class="{ 'border-indigo-500 text-indigo-600 dark:text-indigo-400': activeTab === 'logs', 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300': activeTab !== 'logs' }" class="inline-block p-4 border-b-2 font-medium">
                    Logs
                </button>
            </li>
            <li class="mr-2">
                <button @click="activeTab = 'script'" :class="{ 'border-indigo-500 text-indigo-600 dark:text-indigo-400': activeTab === 'script', 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300': activeTab !== 'script' }" class="inline-block p-4 border-b-2 font-medium">
                    Script
                </button>
            </li>
        </ul>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                ...
                <div class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg p-4">
                    <dl class="divide-y divide-zinc-200 dark:divide-zinc-700/50">
                        <div class="py-3 grid grid-cols-3 gap-4">
                            ...
                            <dd class="text-sm text-zinc-900 dark:text-white col-span-2">{{ $execution->testScript->name ?? 'Unknown' }}</dd>
                        </div>
                        <div class="py-3 grid grid-cols-3 gap-4">
                            <dt class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Test Case</dt>
                            <dd class="text-sm text-zinc-900 dark:text-white col-span-2">{{ $execution->testScript->testCase->title ?? 'N/A' }}</dd>
                        </div>
                        <div class="py-3 grid grid-cols-3 gap-4">
                            <dt class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Environment</dt>
                            <dd class="text-sm text-zinc-900 dark:text-white col-span-2">{{ $execution->environment->name ?? 'Unknown' }}</dd>
                        </div>
                        <div class="py-3 grid grid-cols-3 gap-4">
                            <dt class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Framework</dt>
                            <dd class="text-sm text-zinc-900 dark:text-white col-span-2">{{ ucfirst(str_replace('-', ' ', $execution->testScript->framework_type ?? 'Unknown')) }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg p-4">
                    <ol class="relative border-l border-zinc-300 dark:border-zinc-600 ml-3">
                        <li class="mb-6 ml-6">
                            <span class="absolute flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full -left-3 ring-8 ring-zinc-50 dark:ring-zinc-700/30 dark:bg-blue-900/30">
                        ...
                        @if($execution->end_time)
                            <li class="ml-6">
                                ...
                                @endphp
                                <span class="absolute flex items-center justify-center w-6 h-6 {{ $iconClass }} rounded-full -left-3 ring-8 ring-zinc-50 dark:ring-zinc-700/30">
        ...
        <div x-show="activeTab === 'containers'" class="p-6">
            ...
            @if($execution->containers->isEmpty())
                <div class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg p-6 text-center">
                    ...
                    <p class="mt-2 text-zinc-500 dark:text-zinc-400">This execution hasn't created any containers yet.</p>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-700/30">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Container ID</th>
                            ...
                            @foreach($execution->containers as $container)
                                @php
                                    ...
                                    $statusClass = match($containerStatus) {
                                        ...
                                        'running' => 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 animate-pulse',
                                        'completed' => 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300',
                                        'failed', 'terminated' => 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300',
                                        default => 'bg-zinc-100 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-300',
                                    };

                                    $containerDuration = null;
                                    if ($container->start_time && $container->end_time) {
                                        $containerDuration = $container->start_time->diffInSeconds($container->end_time);
                                    }
                                @endphp
                                <tr>
        ...
        <div x-show="activeTab === 'logs'" class="p-6">
            ...
            @elseif($execution->isRunning())
                <div class="bg-zinc-50 dark:bg-zinc-700/30 rounded-lg p-6 text-center">
                    <div class="animate-spin mb-3 mx-auto w-12 h-12 border-4 border-indigo-500 dark:border-indigo-400 border-t-transparent dark:border-t-transparent rounded-full"></div>
                    <h3 class="text-lg font-medium text-zinc-900 dark:text-white">Execution in Progress</h3>
                    <p class="mt-2 text-zinc-500 dark:text-zinc-400">Logs will be available when the execution completes.</p>
                </div>
            @else
                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg overflow-hidden">
                    <div class="flex items-center justify-between bg-zinc-100 dark:bg-zinc-700 px-4 py-2">
                        ...
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('dashboard.executions.logs.download', $execution->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 text-sm">
