@php
    ...
    $baseSuiteUrl = $isGenericIndex ? url('dashboard/projects') : route('dashboard.projects.test-suites.index', $project->id);
    ...
    <div x-data="testSuiteEnhanced" x-init="initNotifications()" class="relative space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex-shrink-0">
                ...
                @else
                    <span class="inline-flex items-center px-5 py-2.5 bg-zinc-200 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400 font-medium rounded-xl cursor-not-allowed"
        ...
        @if ($isGenericIndex && isset($projects)) {{-- Ensure $projects is passed for generic view --}}
            <div class="animate-fade-in-down" x-data="projectFilterDropdown({ currentProjectId: '{{ $currentProjectId }}', projects: {{ json_encode($projects->toArray()) }} })">
                    ...
                    <div class="relative dashboard-container" id="project-select-container">
                        ...
                        <button type="button" @click="toggleDropdown" @keydown.escape.prevent="closeDropdown()"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <span x-text="selectedProjectName || 'Filter by Project...'" class="text-zinc-700 dark:text-zinc-200"></span>
                            </div>
                            <i data-lucide="chevron-down" class="w-5 h-5 text-zinc-400 transition-transform duration-200" :class="{ 'rotate-180': isOpen }"></i>
                        </button>
                        ...
                        <div x-show="isOpen" @click.outside="closeDropdown"
                             ...
                             class="dropdown-menu"
                             role="listbox" aria-labelledby="project-select-trigger" x-trap.inert.noscroll="isOpen">
                            {{-- Search Input --}}
                            <div class="p-2 border-b border-zinc-100 dark:border-zinc-700">
                                <div class="relative">
                                    <i data-lucide="search" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none"></i>
                                    <input type="search" x-model="searchTerm" @input="filterProjects" placeholder="Search projects..."
                                ...
                                <ul>
                                    ...
                                    <li role="option" :aria-selected="selectedProjectId === ''">
                                        <button type="button" @click="selectProject('', 'All Projects')"
                                            ...
                                            <i data-lucide="check" class="w-4 h-4 text-indigo-600 dark:text-indigo-400" x-show="selectedProjectId === ''"></i>
                                        </button>
                                    </li>
                                    ...
                                    <template x-for="project in filteredProjects" :key="project.id">
                                        <li role="option" :aria-selected="selectedProjectId === project.id">
                                            <button type="button" @click="selectProject(project.id, project.name)"
                                                class="w-full px-4 py-2.5 text-left text-sm flex items-center justify-between hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors"
                                                :class="{ 'font-semibold text-indigo-600 dark:text-indigo-400': selectedProjectId === project.id }">
                                                <span x-text="project.name"></span>
                                                <i data-lucide="check" class="w-4 h-4 text-indigo-600 dark:text-indigo-400" x-show="selectedProjectId === project.id"></i>
                                            </button>
                                        </li>
                                    </template>
                                     {{-- No Results Message --}}
                                     <template x-if="filteredProjects.length === 0">
                                        <li class="px-4 py-3 text-sm text-center text-zinc-500 dark:text-zinc-400">No projects found.</li>
                                    </template>
                                </ul>
                            </div>
                        </div>
        ...
        <div class="transition-opacity duration-500 ease-in-out">
            @if ($testSuites->isEmpty())
                ...
                <div class="text-center py-16 px-6 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border-2 border-dashed border-zinc-200 dark:border-zinc-700/50">
                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 mb-5 text-indigo-600 dark:text-indigo-400">
                        ...
                        <div x-data="{ showActions: false }" @mouseenter="showActions = true" @mouseleave="showActions = false"
                            ...
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    {{-- Metadata --}}
                                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-zinc-500 dark:text-zinc-400/80">
                                        <span class="inline-flex items-center">
                                            <i data-lucide="file-check-2" class="w-3.5 h-3.5 mr-1 text-green-500"></i>
                                            {{ $suite->test_cases_count }} {{ Str::plural('Case', $suite->test_cases_count) }}
                                        </span>
                                        <span class="inline-flex items-center">
                                         ...
                                         <span class="inline-flex items-center capitalize {{
                                            match(strtolower($suite->settings['default_priority'] ?? 'medium')) {
                                                'high' => 'text-red-500',
                                                'low' => 'text-green-500',
                                ...
                                <div class="flex flex-col items-end md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-2 flex-shrink-0 transition-opacity duration-200"
                                    ...
                                    <a href="{{ route('dashboard.projects.test-suites.show', [$suite->project_id, $suite->id]) }}"
                                       class="action-btn text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30" title="View Details">
                                        <i data-lucide="eye" class="w-4 h-4"></i> <span class="hidden md:inline ml-1">View</span>
                                    </a>
                                    <a href="{{ route('dashboard.projects.test-suites.edit', [$suite->project_id, $suite->id]) }}"
                                       class="action-btn text-yellow-600 dark:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-900/30" title="Edit Suite">
                                        <i data-lucide="pencil" class="w-4 h-4"></i> <span class="hidden md:inline ml-1">Edit</span>
                                    </a>
                                    <button @click="openDeleteModal('{{ $suite->id }}', '{{ addslashes($suite->name) }}', '{{ $suite->project_id }}')"
                                            class="action-btn text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Delete Suite">
                                        <i data-lucide="trash-2" class="w-4 h-4"></i> <span class="hidden md:inline ml-1">Delete</span>
                                    </button>
                                </div>
                            </div>
                        </div>
        ...
        <div x-show="showDeleteModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
                     ...
                     <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-zinc-600 dark:text-zinc-400">
                                        Are you sure you want to delete the test suite "<strong class="font-semibold text-zinc-700 dark:text-zinc-200" x-text="deleteSuiteName"></strong>"? This will also delete all associated test cases. This action cannot be undone.
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        <!-- Notification Toast -->
        <div x-show="showNotification" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-4" class="fixed bottom-6 right-6 z-[100] max-w-sm w-full" style="display: none;">
             <div class="shadow-lg border rounded-xl p-4 backdrop-blur-sm"
                ...
                <div class="flex items-start">
                    <i data-lucide="check-circle" x-show="notificationType === 'success'" class="w-5 h-5 mr-3 text-green-600 dark:text-green-400 flex-shrink-0"></i>
                    <i data-lucide="alert-circle" x-show="notificationType === 'error'" class="w-5 h-5 mr-3 text-red-600 dark:text-red-400 flex-shrink-0"></i>
                    <div class="flex-1">
                    ...
                    <button @click="hideNotification" class="ml-3 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200">
...
@push('styles')
<style>
    .btn-primary { @apply bg-zinc-800 hover:bg-zinc-700 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition duration-150 ease-in-out disabled:opacity-50; }
    .action-btn { @apply p-1.5 rounded-md transition-colors duration-150; }

    /* Style for the dropdown menu */
    #project-select-dropdown { transform-origin: top center; }
    /* Scrollbar styling for dark mode */
    .dark #project-select-dropdown ul { scrollbar-width: thin; scrollbar-color: rgba(161, 161, 170, 0.5) rgba(63, 63, 70, 0.1); }
    .dark #project-select-dropdown ul::-webkit-scrollbar { width: 6px; }
    .dark #project-select-dropdown ul::-webkit-scrollbar-track { background: rgba(63, 63, 70, 0.1); border-radius: 3px;}
    .dark #project-select-dropdown ul::-webkit-scrollbar-thumb { background-color: rgba(161, 161, 170, 0.5); border-radius: 3px; }
    .dark #project-select-dropdown ul::-webkit-scrollbar-thumb:hover { background-color: rgba(161, 161, 170, 0.7); }
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('testSuiteEnhanced', () => ({
                ...
                async confirmDelete() {
                    ...
                    try {
                        ...
                        const deleteUrl = `/dashboard/projects/${this.deleteProjectId}/test-suites/${this.deleteSuiteId}`;
                        ...
                        if (response.ok && result.success) {
                            ...
                            const suiteElement = document.getElementById(`suite-card-${this.deleteSuiteId}`); // Assuming you add an ID like this to the card
                            if (suiteElement) {
                                suiteElement.remove();
                            }
            ...
            Alpine.data('projectFilterDropdown', (config) => ({
                ...
                init() {
                    ...
                    const currentProject = this.allProjects.find(p => p.id === this.selectedProjectId);
                    this.selectedProjectName = currentProject ? currentProject.name : 'All Projects';

                     this.$nextTick(() => {
                        if (typeof lucide !== 'undefined') {
                            lucide.createIcons(); // Ensure icons in dropdown are rendered
                        }
                    });
                },
