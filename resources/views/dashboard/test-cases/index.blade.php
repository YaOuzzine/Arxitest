...
<div x-data="testCasesManager({
        ...
        @if ($isGenericIndex)
            projects:   {{ json_encode($projectsForFilter->toArray()) }},
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex-shrink-0">
            ...
            @else
                <a href="{{ route('dashboard.projects.test-suites.test-cases.create', [$selectedProjectId, $selectedSuiteId]) }}{{ $storyParam }}"
                    class="btn-primary inline-flex items-center px-5 py-2.5 group">
                    <i data-lucide="plus-circle"
                        class="w-5 h-5 mr-2 transition-transform duration-200 group-hover:rotate-90"></i>
                    New Test Case
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        ...
                                        <template x-for="project in filteredProjects" :key="project.id">
                                            <li>
                                                <button type="button" @click="selectProject(project.id, project.name)"
                                                    class="w-full text-left px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-700"
                                                    :class="{ 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300': selectedProjectId === project.id }">
                    ...
                    @if ($isGenericIndex || $isProjectIndex)
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Story</label>
                            <div class="dropdown-container" x-data="{ open: false }">
                                <button type="button" @click="open = !open" @keydown.escape="open = false"
                                        ...
                                        <template x-for="story in filteredStories" :key="story.id">
                                            <li>
                                                <button type="button" @click="selectStory(story.id, story.title)"
                                                    class="w-full text-left px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-700"
                                                    :class="{ 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300': selectedStoryId === story.id }">
                    ...
                    @if ($isGenericIndex || $isProjectIndex)
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Test Suite</label>
                            <div class="dropdown-container" x-data="{ open: false }">
                                <button type="button" @click="open = !open" @keydown.escape="open = false"
                                    :disabled="isGenericIndex && !selectedProjectId"
                                    class="w-full flex items-center justify-between px-4 py-2 bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
                                        ...
                                        <template x-for="suite in filteredSuites" :key="suite.id">
                                            <li>
                                                <button type="button" @click="selectSuite(suite.id, suite.name)"
                                                    class="w-full text-left px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-700"
                                                    :class="{ 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300': selectedSuiteId === suite.id }">
        ...
        <div class="p-6 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                ...
                <select @change="updateSort($event)"
                    ...
                    <option value="updated_at" {{ $sortField === 'updated_at' ? 'selected' : '' }}>Last Updated</option>
                    <option value="created_at" {{ $sortField === 'created_at' ? 'selected' : '' }}>Date Created</option>
                </select>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('testCasesManager', (config) => ({
        ...
        async fetchStoriesForProject(projectId) {
            try {
                const res = await fetch(`/projects/${projectId}/stories`);
                const json = await res.json();
        ...
        async confirmDelete() {
            ...
            try {
                ...
                if (this.isSuiteIndex) {
                    deleteUrl = `/dashboard/projects/${this.projectId}/test-suites/${this.suiteId}/test-cases/${this.deleteItemId}`;
                } else if (this.isProjectIndex) {
                    deleteUrl = `/dashboard/projects/${this.projectId}/test-cases/${this.deleteItemId}`;
                } else {
                    ...
                    deleteUrl = `/dashboard/projects/${this.deleteProjectId}/test-cases/${this.deleteItemId}`;
                }
