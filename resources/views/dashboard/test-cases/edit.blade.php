@php
     ...
     * @var \App\Models\TestSuite $selectedSuite // This is the currently selected suite for the test case
     */
    ...
    $selectedSuiteName = $testSuites->firstWhere('id', $selectedSuiteId)?->name ?? 'Select Suite'; // Find name based on ID
    $selectedPriority = old('priority', $testCase->priority);
    $selectedStatus = old('status', $testCase->status);
    $displaySteps = old('steps', $currentSteps); // Use old steps if available, otherwise current
    $displayTags = old('tags', $currentTags);   // Use old tags if available, otherwise current

    // Ensure displaySteps is always an array for the loop, add an empty element if empty for initial display
    if (!is_array($displaySteps)) { $displaySteps = []; }
    if (empty($displaySteps)) { $displaySteps = ['']; }

     // Ensure displayTags is always an array
     if (!is_array($displayTags)) { $displayTags = []; }
...
@section('content')
<div class="h-full" id="test-case-edit-container" data-project-id="{{ $project->id }}"
    x-data="{
        currentSuiteId: '{{ $selectedSuiteId }}',
        currentSuiteName: '{{ addslashes($selectedSuiteName) }}',
        manualSuiteDropdownOpen: false,
        priority: '{{ $selectedPriority }}', // Add priority state
        status: '{{ $selectedStatus }}', // Add status state

        updateSuite(id, name) {
            this.currentSuiteId = id;
            this.currentSuiteName = name;
            this.manualSuiteDropdownOpen = false;
            // No need to manually update hidden input if using x-model directly
        },
         // Helper to check if priority matches current state
        isPriority(value) {
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div class="space-y-1">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">
                    Editing "<span class="font-medium">{{ $testCase->title }}</span>" in suite "<span x-text="currentSuiteName"></span>"
                </p>
            </div>
            <div>
                <a href="{{ route('dashboard.projects.test-cases.show', [$project->id, $testCase->id]) }}" class="group inline-flex items-center px-4 py-2.5 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm text-sm font-medium text-zinc-700 dark:text-zinc-200 bg-white/70 dark:bg-zinc-800/50 hover:bg-white dark:hover:bg-zinc-700/50 backdrop-blur-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
    ...
    <div class="bg-white/70 dark:bg-zinc-800/50 rounded-2xl shadow-2xl border border-zinc-200/50 dark:border-zinc-700/30 backdrop-blur-lg transition-all duration-300 hover:shadow-2xl">
        <div class="p-8">
            <form id="test-case-form" method="POST" action="{{ route('dashboard.projects.test-cases.update', [$project->id, $testCase->id]) }}">
                @csrf
                @method('PUT') {{-- Use PUT for updates --}}

                {{-- Suite ID Input (bound to Alpine) --}}
                <input type="hidden" name="suite_id" id="suite-id-input" x-model="currentSuiteId">
                {{-- Priority Input (bound to Alpine) --}}
                <input type="hidden" name="priority" id="priority-input" x-model="priority">
                 {{-- Status is handled by radios below, ensure name="status" is used --}}
                ...
                <div id="test-case-form-fields" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-6">
                        ...
                        <div class="sm:col-span-3 animate-fade-in-up dropdown-container relative" id="manual-suite-selection-container">
                            ...
                            <div class="flex flex-wrap gap-x-4 gap-y-2">
                                @foreach (['draft', 'active', 'deprecated', 'archived'] as $statusValue)
                                    <div class="status-option relative flex items-center">
                                        <input type="radio" id="status-{{ $statusValue }}" name="status" value="{{ $statusValue }}"
                                               @checked($selectedStatus === $statusValue) {{-- Use checked --}}
                                               class="form-radio h-4 w-4 text-indigo-600 border-zinc-300 dark:border-zinc-600 focus:ring-indigo-500">
                                        <label for="status-{{ $statusValue }}" class="ml-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
                                            {{ ucfirst($statusValue) }}
                                        </label>
                                    </div>
                                @endforeach
                        ...
                        <div class="sm:col-span-6 animate-fade-in-up">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4" id="priority-options">
                                ...
                                <div @click="setPriority('low')"
                                     ...
                                     :class="{ 'ring-2 ring-zinc-500 dark:ring-zinc-400 bg-zinc-100/50 dark:bg-zinc-700/30': isPriority('low') }">
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div class="flex items-center h-5 ml-2">
                                            <div class="priority-radio bg-zinc-300 dark:bg-zinc-600 w-4 h-4 rounded-full transition-colors duration-300 relative"
                                                 ...
                                                 <div x-show="isPriority('low')" class="absolute inset-0 flex items-center justify-center transform">
                                ...
                                <div @click="setPriority('medium')"
                                     class="priority-option relative p-4 rounded-xl cursor-pointer transition-all duration-300 group ring-1 ring-zinc-200/70 dark:ring-zinc-600/50 hover:ring-zinc-300 dark:hover:ring-zinc-500"
                                     :class="{ 'ring-2 ring-zinc-500 dark:ring-zinc-400 bg-zinc-100/50 dark:bg-zinc-700/30': isPriority('medium') }">
                                    <div class="flex items-start space-x-3">
                                        <div class="flex-1">
                                            <div class="flex items-center">
                                        ...
                                        <div class="flex items-center h-5 ml-2">
                                            <div class="priority-radio bg-zinc-300 dark:bg-zinc-600 w-4 h-4 rounded-full transition-colors duration-300 relative"
                                                 ...
                                                 <div x-show="isPriority('medium')" class="absolute inset-0 flex items-center justify-center transform">
                                                     <div class="w-2 h-2 rounded-full bg-white dark:bg-zinc-800"></div>
                                                 </div>
                                            </div>
                                        </div>
                                ...
                                <div @click="setPriority('high')"
                                     class="priority-option relative p-4 rounded-xl cursor-pointer transition-all duration-300 group ring-1 ring-zinc-200/70 dark:ring-zinc-600/50 hover:ring-zinc-300 dark:hover:ring-zinc-500"
                                     :class="{ 'ring-2 ring-zinc-500 dark:ring-zinc-400 bg-zinc-100/50 dark:bg-zinc-700/30': isPriority('high') }">
                                    <div class="flex items-start space-x-3">
                                        <div class="flex-1">
                                            <div class="flex items-center">
                                        ...
                                        <div class="flex items-center h-5 ml-2">
                                            <div class="priority-radio bg-zinc-300 dark:bg-zinc-600 w-4 h-4 rounded-full transition-colors duration-300 relative"
                                                 ...
                                                 <div x-show="isPriority('high')" class="absolute inset-0 flex items-center justify-center transform">
                                                     <div class="w-2 h-2 rounded-full bg-white dark:bg-zinc-800"></div>
                                                 </div>
                                            </div>
                                        </div>
...
<style>
    ...
    .dropdown-menu { /* Base styles */
        position: absolute; z-index: 20; margin-top: 0.25rem; width: 100%;
    ...
    .dropdown-item { padding: 0.5rem 1rem; font-size: 0.875rem; cursor: pointer; display: flex; align-items: center; }
...
@push('scripts')
{{-- Include AlpineJS if not already global --}}
{{-- <script src="//unpkg.com/alpinejs" defer></script> --}}
{{-- Reuse the same JS file as create, assuming it's adapted --}}
{{-- @vite('resources/js/test-case-create.js') --}}

{{-- Inlined JS specific for edit view - can be moved to a dedicated file --}}
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('testCaseEditForm', (config) => ({
            ...
            status: config.status, // Initial status from PHP
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        priorityOptions.forEach(option => {
            option.addEventListener('click', function() {
                if (priorityInput) priorityInput.value = this.dataset.value; // Update hidden input
                ...
                currentRadio.innerHTML = '<div class="absolute inset-0 flex items-center justify-center transform"><div class="w-2 h-2 rounded-full bg-white dark:bg-zinc-800"></div></div>'; // Add inner dot
            });
        });
        ...
        function updateStepNumbers() {
            ...
            removeButtons.forEach(btn => {
                if (canRemove) {
                    ...
                    btn.classList.remove('disabled:opacity-50', 'disabled:cursor-not-allowed', 'disabled:hover:bg-transparent', 'disabled:hover:text-zinc-500');
                } else {
                    btn.setAttribute('disabled', 'disabled');
                     btn.classList.add('disabled:opacity-50', 'disabled:cursor-not-allowed', 'disabled:hover:bg-transparent', 'disabled:hover:text-zinc-500');
                }
            });
        }
        ...
        if (addStepBtn && stepsList) {
            addStepBtn.addEventListener('click', function() {
                ...
                newStep.className = 'step-item flex items-start space-x-3 animate-fade-in'; // Added animation
                newStep.innerHTML = `
                    <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 font-semibold text-sm">
                ...
                updateStepNumbers(); // Renumber and manage remove buttons
                 if (typeof lucide !== 'undefined') {
                     lucide.createIcons({ elements: [newStep.querySelector('.remove-step-btn')] });
                 }
            });
        }
        ...
        const tagInputContainer = document.getElementById('tag-input-container'); // Reference to input's container
        ...
        function addTagFromInput() {
            ...
            const existingTags = Array.from(tagsContainer.querySelectorAll('input[name="tags[]"]')).map(input => input.value.toLowerCase());
            ...
            tagElement.className = 'tag-item inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-200 border border-indigo-200/50 dark:border-indigo-800/50 animate-fade-in';
            ...
            if (typeof lucide !== 'undefined') {
                lucide.createIcons({ elements: [tagElement.querySelector('.remove-tag-btn i')] });
            }
        }

        if (addTagBtn) addTagBtn.addEventListener('click', addTagFromInput);
        if (tagInput) tagInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); addTagFromInput(); } });
        if (tagsContainer) tagsContainer.addEventListener('click', (e) => {
            const removeBtn = e.target.closest('.remove-tag-btn');
            if (removeBtn) { removeBtn.closest('.tag-item')?.remove(); }
        });
        ...
        if (form && submitBtn && submitBtnText) {
            form.addEventListener('submit', function(e) {
                ...
                const suiteId = document.getElementById('suite-id-input').value; // Check hidden input
                const steps = Array.from(document.querySelectorAll('.step-input')).map(input => input.value.trim()).filter(s => s); // Filter empty steps
                ...
                let isValid = true;
                if (!title) { console.error("Title missing"); isValid = false; }
                if (!suiteId) { console.error("Suite ID missing"); isValid = false; }
                if (steps.length === 0) { console.error("Steps missing"); isValid = false; }
                if (!expectedResults) { console.error("Expected Results missing"); isValid = false; }
                if (!statusSelected) { console.error("Status missing"); isValid = false; }
                ...
                spinner.innerHTML = `<svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>`;
    ...
    function showNotification(type, title, message) {
        ...
        const colorMap = {
            success: { bg: 'bg-green-50/80 border-green-200/50 dark:bg-green-900/30 dark:border-green-800/30', text: 'text-green-800 dark:text-green-200', icon: 'text-green-600 dark:text-green-400' },
            error: { bg: 'bg-red-50/80 border-red-200/50 dark:bg-red-900/30 dark:border-red-800/30', text: 'text-red-800 dark:text-red-200', icon: 'text-red-600 dark:text-red-400' }
        };
        container.className = `fixed bottom-6 right-6 z-50 max-w-sm w-full shadow-lg border rounded-xl p-4 ${colorMap[type]?.bg || ''}`;
        titleEl.className = `font-medium mb-1 ${colorMap[type]?.text || ''}`;
        messageEl.className = `text-sm ${colorMap[type]?.text || ''} opacity-90`;
        iconEl.innerHTML = `<i data-lucide="${iconMap[type] || 'info'}" class="w-5 h-5 ${colorMap[type]?.icon || ''}"></i>`;
        container.classList.remove('hidden');
        if (typeof lucide !== 'undefined') { lucide.createIcons({ elements: [iconEl] }); }
