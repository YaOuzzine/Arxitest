...
@section('breadcrumbs')
    ...
    <li class="flex items-center">
        <i data-lucide="chevron-right" class="w-4 h-4 text-zinc-400 mx-1"></i>
        <a href="{{ route('dashboard.environments.show', $environment->id) }}" class="text-zinc-700 dark:text-zinc-300">{{ $environment->name }}</a>
    </li>
    <li class="flex items-center">
        <i data-lucide="chevron-right" class="w-4 h-4 text-zinc-400 mx-1"></i>
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-6">
        <h1 class="text-2xl font-bold text-zinc-900 dark:text-white mb-6">Edit Environment</h1>

        <form action="{{ route('dashboard.environments.update', $environment->id) }}" method="POST" x-data="{
            isGlobal: {{ $environment->is_global ? 'true' : 'false' }},
            configurations: [],
            addConfig() {
            ...
            init() {
                // Convert configuration object to array of key-value pairs
                @if(is_array($environment->configuration))
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $environment->name) }}"
                ...
                <div x-show="!isGlobal" x-transition:enter="transition-opacity ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    ...
                    <select id="projects" name="projects[]" multiple
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" {{ in_array($project->id, old('projects', $selectedProjects)) ? 'selected' : '' }}>
                                {{ $project->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex justify-end mt-8">
                    <a href="{{ route('dashboard.environments.show', $environment->id) }}" class="btn-secondary mr-3">
                        Cancel
                    </a>
