...
<head>
    ...
    <style>
        ...
        /* Dropdown */
        .dropdown-menu {
            @apply absolute right-0 mt-2 w-56 origin-top-right bg-white dark:bg-zinc-800 rounded-md shadow-xl ring-1 ring-black dark:ring-zinc-700 ring-opacity-5 focus:outline-none z-50;
        }

        .dropdown-item {
            @apply block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors;
        }
...
<body
    ...
    <div x-data="dashboardLayout({
        ...
        <aside x-show="sidebarOpen" x-transition:enter="sidebar-transition"
            ...
            x-transition:enter-end="opacity-100 transform translate-x-0" x-transition:leave="sidebar-transition"
            x-transition:leave-start="opacity-100 transform translate-x-0"
            x-transition:leave-end="opacity-0 transform -translate-x-full" @click.away="closeSidebarOnMobile()"
            ...
            <div
                class="h-16 flex items-center justify-between px-4 border-b border-zinc-200 dark:border-zinc-700/50 flex-shrink-0">
            ...
            <div class="flex flex-col flex-grow overflow-y-auto custom-scrollbar">
                <nav class="flex-1 px-2 py-4 space-y-3"> {{-- Adjusted padding/spacing --}}
                    {{-- Main Section --}}
                    <div>
                        <h3 class="px-3 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2"
                            id="main-nav-heading">Main</h3>
                    ...
                    {{-- Settings Section --}}
                    <div>
                        <h3 class="px-3 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2 mt-4"
                            id="settings-nav-heading">Management</h3>
                            ...
                            <li>
                                <x-nav-link :route="'dashboard.teams.index'" icon="users-round">
                                    ...
                                    @if ($pendingInvitationCount > 0)
                                        <x-slot name="badge">
                                            <span
                                                class="ml-auto inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 text-[10px] font-semibold leading-none rounded-full bg-indigo-500 text-white">
                ...
                <div class="mt-auto px-4 pb-4 pt-2 animate-pop-in" style="--delay: 0.5s;">
                    ...
                    @isset($activeSubscription)
                        @php
                            ...
                            $usagePercentage =
                                $maxContainers > 0
                                    ? min(100, round(($currentUsage / $maxContainers) * 100))
                                    : ($maxContainers === 0
                                        ? 0
                                        : 100); // Handle unlimited (0) or errors
                            $usageColor =
                                $usagePercentage >= 90
                                    ? 'bg-red-500'
                                    : ($usagePercentage >= 70
                                        ? 'bg-yellow-500'
                                        : 'bg-teal-500');
                            $planName = $activeSubscription->plan_type
                                ? ucwords(str_replace('_', ' ', $activeSubscription->plan_type))
                                : 'Current';
                        @endphp
                        <div
                            class="bg-gradient-to-br from-zinc-100 to-slate-100 dark:from-zinc-700/60 dark:to-slate-700/50 rounded-xl p-4 border border-zinc-200 dark:border-zinc-600/50 shadow-inner">
                            <div class="flex items-center justify-between mb-2.5">
                                ...
                                @if ($activeSubscription->is_active ?? false)
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 border border-green-200 dark:border-green-700">
                                        <i data-lucide="check" class="w-3 h-3 mr-1"></i> Active
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full bg-zinc-100 text-zinc-800 dark:bg-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-500">
                                        Inactive
                                    </span>
                                @endif
                            </div>

                            @if (!is_null($maxContainers))
                                {{-- Show usage only if max_containers is set --}}
                                <div
                                    class="flex justify-between items-center text-xs text-zinc-500 dark:text-zinc-400 mb-1.5">
                                    <span>Container Usage</span>
                                    <span class="font-medium">{{ $currentUsage }} /
                                        {{ $maxContainers === 0 ? 'Unlimited' : $maxContainers }}</span>
                                </div>
                                <div
                                    class="w-full bg-zinc-200 dark:bg-zinc-600 rounded-full h-1.5 overflow-hidden relative">
                                    {{-- Add subtle stripe pattern to background --}}
                                    <div class="absolute inset-0 opacity-10 dark:opacity-20"
                                        style="background-image: repeating-linear-gradient(-45deg, transparent, transparent 5px, rgba(0,0,0,0.1) 5px, rgba(0,0,0,0.1) 10px);">
                                    </div>
                                    <div class="{{ $usageColor }} h-full rounded-full transition-all duration-500 ease-out"
                            ...
                            <a href="{{ route('dashboard') }}?page=subscription"
                                ...
                                <i data-lucide="arrow-right"
                                    class="ml-1 w-3 h-3 group-hover:translate-x-0.5 transition-transform"></i>
                            </a>
                        </div>
                    @else
                        {{-- Fallback if no subscription data could be loaded --}}
                        <div
                            class="bg-zinc-100 dark:bg-zinc-700/30 rounded-lg p-4 text-center border border-zinc-200 dark:border-zinc-600/50">
                            ...
                            <a href="#" {{-- Link to billing/plans page --}}
                                ...
                                View Billing <i data-lucide="chevron-right"
                                    class="ml-1 w-3 h-3 group-hover:translate-x-0.5 transition-transform"></i>
                            </a>
                        </div>
                    @endisset
        ...
        <div class="flex-1 flex flex-col min-h-screen overflow-x-hidden">
                ...
                <div class="px-4 sm:px-6 lg:px-8 w-full flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-3">
                        ...
                        <button @click="toggleTheme" aria-label="Toggle theme"
                            ...
                            <i data-lucide="sun" class="w-5 h-5 transition-all duration-300 transform"
                                :class="{ 'rotate-90 scale-0': isDarkMode }"></i>
                            <i data-lucide="moon" class="top-5 w-5 h-5 absolute transition-all duration-300 transform"
                                :class="{ 'rotate-0 scale-100': isDarkMode, '-rotate-90 scale-0': !isDarkMode }"></i>
                        </button>
                                    ...
                                    <template
                                        ...
                                        <div class="px-4 py-10 text-center">
                                            <i data-lucide="bell-off"
                                                class="w-10 h-10 mx-auto text-zinc-400 dark:text-zinc-500 mb-2"></i>
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400">No notifications yet.
                                            </p>
                                        </div>
                                    </template>
            ...
            {{-- Footer --}}
            <footer
                class="flex-shrink-0 px-4 sm:px-6 lg:px-8 py-3 border-t border-zinc-200 dark:border-zinc-700/50 text-center text-xs text-zinc-500 dark:text-zinc-400">
                © {{ date('Y') }} {{ config('app.name', 'Arxitest') }}. All rights reserved. | <a href="#"
                    class="hover:underline">Privacy Policy</a> | <a href="#" class="hover:underline">Terms of
                    Service</a>
            </footer>
        </div>
    </div>
